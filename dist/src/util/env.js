...
function _getEnvironment() {
    return process.env.NODE_ENV ? process.env.NODE_ENV : require('os').platform() == 'darwin' || require('os').platform() == 'win32' ? 'local' : 'develop';
}
function _getConfig() {
    var configs = require_1.reqDir(process.cwd() + "/config");
    var defConfig = Object.keys(configs).reduce(function (config, key) {
